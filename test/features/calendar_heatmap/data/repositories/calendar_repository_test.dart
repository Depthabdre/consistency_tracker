...
class MockCalendarLocalDataSource extends Mock implements CalendarLocalDataSource {}
...
void main() {
  ...
  group('CalendarRepository - addFocusMinutesToToday QA Skill Tests', () {
    test('TC-P0-01 (Defect Fix): Goal with 1 min progress + 2 min focus session stop equals 3 mins (isCompleted == false)', () async {
      ...
      when(() => mockLocalDataSource.addFocusMinutesToToday(
            goalId: 'goal_1',
            targetMinutes: 25,
            minutesToAdd: 2,
    ...
    test('TC-P0-02 (Target Completed): 20 mins progress + 5 mins focus session equals 25 mins (isCompleted == true)', () async {
      final expectedDay = CalendarDayModel(
        date: DateTime(today.year, today.month, today.day),
    ...
    test('TC-P1-01 (Boundary 0s): 1 min progress + 0 min focus session equals 1 min (isCompleted == false)', () async {
      final expectedDay = CalendarDayModel(
        date: DateTime(today.year, today.month, today.day),
    ...
    test('TC-P2-01 (Negative Failure): Storage error maps to CacheFailure cleanly', () async {
      when(() => mockLocalDataSource.addFocusMinutesToToday(
            goalId: 'goal_1',
            targetMinutes: 25,
            minutesToAdd: 2,
