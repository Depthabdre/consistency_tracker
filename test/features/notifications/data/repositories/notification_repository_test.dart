...
void main() {
  ...
  group('NotificationRepository QA Skill Unit Tests', () {
    test(
      'TC-P0-01 (Positive): initNotifications initializes local data source',
      () async {
    ...
    test(
      'TC-P0-02 (Positive): scheduleGoalReminders schedules all active reminder times',
      () async {
        when(
    ...
    test(
      'TC-N1-01 (Negative): Exception during scheduleGoalReminders maps cleanly to NotificationFailure',
      () async {
        when(
          () => mockLocalDataSource.scheduleGoalNotification(
            id: any(named: 'id'),
        ...
        result.fold(
          ...
          onFailure: (failure) {
            expect(
              failure.message,
              contains('Failed to schedule goal reminders'),
            );
          },
        );
      },
    ...
    test(
      'TC-C0-01 (Monkey/Chaos): Goal deletion triggers cancelGoalReminders',
      () async {
        when(
          () => mockLocalDataSource.cancelGoalReminders('g1'),
        ).thenAnswer((_) async {});
