...
void main() {
  ...
  group('ManageTimerUseCase Unit Tests', () {
    test('Positive: buildTargetEndTime adds durationSeconds to from DateTime', () {
      final now = DateTime(2026, 8, 1, 10, 0, 0);
      final targetEnd = useCase.buildTargetEndTime(from: now, durationSeconds: 1500);

      expect(targetEnd, equals(DateTime(2026, 8, 1, 10, 25, 0)));
    });

    test('Positive: calculateRemainingSeconds returns accurate remaining difference', () {
      final now = DateTime(2026, 8, 1, 10, 10, 0);
      final targetEnd = DateTime(2026, 8, 1, 10, 25, 0);

      final remaining = useCase.calculateRemainingSeconds(now: now, targetEndTime: targetEnd);

      expect(remaining, equals(900));
    });

    test('Boundary: calculateRemainingSeconds clamps negative difference to 0', () {
