...
void main() {
  ...
  group('Integration - Defect Prevention (1 min + 2 min = 3 min)', () {
    test(
      'TC-P0-01 (Defect Fix): Goal with 1 min existing progress + 2 min focus session stop = EXACTLY 3/25 mins (NOT 26/25)',
      () async {
        // 1. Initial Load
        ...
        goalBloc.add(
          const UpdateGoalProgressEvent(goalId: 'goal_a', todayMinutes: 1),
        );
        await expectLater(
          goalBloc.stream,
          emitsThrough(
        ...
        goalBloc.add(
          const UpdateGoalProgressEvent(goalId: 'goal_a', todayMinutes: 3),
        );
        await expectLater(
          goalBloc.stream,
          emitsThrough(
  ...
  group('Integration - Positive Multi-Goal Focus Isolation', () {
    test(
      'Focusing 1 minute on Goal A updates ONLY Goal A and leaves Goal B at 0',
      () async {
        goalBloc.add(const LoadGoalsEvent());
        await expectLater(
          goalBloc.stream,
    ...
    test('Completing Goal A target (25 mins) completes ONLY Goal A', () async {
      ...
      goalBloc.add(
        const UpdateGoalProgressEvent(goalId: 'goal_a', todayMinutes: 25),
      );
      await expectLater(
        goalBloc.stream,
        emitsThrough(
      ...
      final state = goalBloc.state as GoalLoadedState;
      expect(
        state.todayMinutesByGoalId['goal_a']! >= goalA.targetMinutes,
        isTrue,
      );
      expect(
        (state.todayMinutesByGoalId['goal_b'] ?? 0) >= goalB.targetMinutes,
        isFalse,
      );
    });
  });
  ...
  group('Integration - Negative & Unusual Edge Cases', () {
    test(
      'Negative: Updating progress for non-existent goal ID does not corrupt existing goals',
      () async {
        goalBloc.add(const LoadGoalsEvent());
        await expectLater(
          goalBloc.stream,
    ...
    test(
      'Edge Case: Extremely large focus minutes (720 mins) handled safely',
      () async {
        goalBloc.add(const LoadGoalsEvent());
        await expectLater(
          goalBloc.stream,
        ...
        goalBloc.add(
          const UpdateGoalProgressEvent(goalId: 'goal_a', todayMinutes: 720),
        );
        await expectLater(
          goalBloc.stream,
          emitsThrough(
  ...
  group('Integration - Monkey / Chaos Testing Scenarios', () {
    test(
      'Chaos: Rapid interleaved progress updates, goal deletion, and reloads retain consistency',
      () async {
        goalBloc.add(const LoadGoalsEvent());
        await expectLater(
          goalBloc.stream,
        ...
        goalBloc.add(
          const UpdateGoalProgressEvent(goalId: 'goal_a', todayMinutes: 5),
        );
        goalBloc.add(
          const UpdateGoalProgressEvent(goalId: 'goal_b', todayMinutes: 10),
        );
        goalBloc.add(
          const UpdateGoalProgressEvent(goalId: 'goal_a', todayMinutes: 20),
        );
